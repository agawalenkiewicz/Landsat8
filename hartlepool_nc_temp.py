...
filename = Landsat_ncfiles.hartlepool
...
result_array = np.zeros((2, 401, 401))
for i, element in enumerate(filename):
	element_path = os.path.join('/glusterfs/surft/users/mp877190/data/datastore/EE/LANDSAT_8_C1/hartlepool_checked', element)
...
sum = result_array[0,:,:] + result_array[1,:,:]
print sum
...
for element in filename:
	...
	aga_outflow_b10.append(nc_file.variables['BT_band10'][out_lat_idx,out_lon_idx])
	aga_outflow_b11.append(nc_file.variables['BT_band11'][out_lat_idx,out_lon_idx])
...
with open('hartlepool_inflow_temp_b10', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(inflow_temp_b10)
with open('hartlepool_inflow_temp_b11', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(inflow_temp_b11)
with open('hartlepool_outflow_temp_b10', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(outflow_temp_b10)
with open('hartlepool_outflow_temp_b11', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(outflow_temp_b11)

with open('hartlepool_outflow_aga_temp_b10', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(aga_outflow_b10)
with open('hartlepool_outflow_aga_temp_b11', 'wb') as myfile:
    wr = csv.writer(myfile)
