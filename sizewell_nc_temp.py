...
filename = Landsat_ncfiles.sizewell
...
result_array = np.zeros((23, 401, 401))
for i, element in enumerate(filename):
	element_path = os.path.join('/glusterfs/surft/users/mp877190/data/datastore/EE/LANDSAT_8_C1/sizewell_checked', element)
...
sum = result_array[0,:,:] + result_array[1,:,:] + result_array[2,:,:] + result_array[3,:,:] + result_array[4,:,:] + result_array[5,:,:] +result_array[6,:,:] +result_array[7,:,:] +result_array[8,:,:] +result_array[9,:,:] +result_array[10,:,:] +result_array[11,:,:] +result_array[12,:,:] +result_array[13,:,:] +result_array[14,:,:] +result_array[15,:,:] +result_array[16,:,:] +result_array[17,:,:] +result_array[18,:,:] +result_array[19,:,:] +result_array[20,:,:] +result_array[21,:,:] +result_array[22,:,:]
print sum
...
for element in filename:
	...
	aga_outflow_b10.append(nc_file.variables['BT_band10'][201,205])
	aga_outflow_b11.append(nc_file.variables['BT_band11'][201,205])
...
with open('sizewell_inflow_temp_b10', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(inflow_temp_b10)
with open('sizewell_inflow_temp_b11', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(inflow_temp_b11)
with open('sizewell_outflow_temp_b10', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(outflow_temp_b10)
with open('sizewell_outflow_temp_b11', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(outflow_temp_b11)

with open('sizewell_outflow_aga_temp_b10', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(aga_outflow_b10)
with open('sizewell_outflow_aga_temp_b11', 'wb') as myfile:
    wr = csv.writer(myfile)
