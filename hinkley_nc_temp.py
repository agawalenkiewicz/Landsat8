...
filename = Landsat_ncfiles.hinkley

#specify the outflow and inflow lat and lon on the commmand line
#in_lat = sys.argv[1]
#in_lon = sys.argv[2]
#out_lat = sys.argv[3]
#out_lon = sys.argv[4]

#create empty lists for inflow and outflow temeperatures
inflow_temp_b10 = []
inflow_temp_b11 = []
outflow_temp_b10 = []
outflow_temp_b11 = []
...
result_array = np.zeros((8, 401, 401))
for i, element in enumerate(filename):
	element_path = os.path.join('/glusterfs/surft/users/mp877190/data/datastore/EE/LANDSAT_8_C1/hinkley_checked', element)
...
sum = result_array[0,:,:] + result_array[1,:,:] + result_array[2,:,:] + result_array[3,:,:] + result_array[4,:,:] + result_array[5,:,:] + result_array[6,:,:] + result_array[7,:,:]
print sum
...
for element in filename:
	...
	aga_outflow_b10.append(nc_file.variables['BT_band10'][203,213])
	aga_outflow_b11.append(nc_file.variables['BT_band11'][203,213])
...
with open('hinkley_inflow_temp_b10', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(inflow_temp_b10)
with open('hinkley_inflow_temp_b11', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(inflow_temp_b11)
with open('hinkley_outflow_temp_b10', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(outflow_temp_b10)
with open('hinkley_outflow_temp_b11', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(outflow_temp_b11)

with open('hinkley_outflow_aga_temp_b10', 'wb') as myfile:
    wr = csv.writer(myfile)
    wr.writerow(aga_outflow_b10)
with open('hinkley_outflow_aga_temp_b11', 'wb') as myfile:
    wr = csv.writer(myfile)
